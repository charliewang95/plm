...
import {EditButton,DeleteButton,CancelButton} from '../vendors/Buttons.js';
import Dialog, {
  DialogActions,
  DialogContent,
  DialogContentText,
...
const Cell = (props)=>{
  console.log(" Column Name " + props.column.name);
  const rowId = props.row.id;
  const onExecute = props.handleChange;

  if(props.column.key == 'order'){
      return <ShoppingCartButton onExecute= {onExecute} rowId = {rowId}/>;
  };
...
const EditCell = ({props, rowId, onExecute}) => {
  return <Cell {...props} />;
  ...
  constructor(props) {
    ...
    this.state = {
      columns: [
        ...
        { key: 'order', title: '',allowFiltering: 'false'},
      ],
...
// Initial loading of data
  componentDidMount() {
    // this.loadInventory();
    this.setState({rows: dummyData.tablePage.inventoryData});
  }
